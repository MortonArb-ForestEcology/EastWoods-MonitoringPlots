# ---------------------------------------
# Mapping & Plot Layout for additional plots sampled in 2017 for URF project
...
candidates <- merge(candidates, candidates2[,c("CORNER", "Lon", "Lat")], all=T)
...
corners <- c("O-109", "O-121", "R-109", "M-134", "T-132", "O-136", "HH-112", "MM-114", "LL-114", "D-123", "A-128", "C-123")

perm <- candidates[candidates$PlotID %in% selected$PlotID,]
selected.pts <- candidates[candidates$CORNER %in% corners,]

perm$Type <- as.factor("Permanent")
selected.pts$Type <- as.factor("URF 2017")

selected.pts <- rbind(perm, selected.pts)
...
selected <- data.frame(selected.pts[,c("Lon", "Lat", "CORNER", "PlotID", "stand", "order", "brn_rgm", "Type")])
selected[,c("x", "y")] <- coordinates(selected.pts)
summary(selected)

png("maps/URF2017_Plot_Locations.png", height=6, width=8, units="in", res=320)
ggplot() +
  ...
  geom_point(data=selected, aes(x=x, y=y, color=Type), size=5) +
  scale_color_manual(values=c("blue2", "orange3")) +
...
png("maps/URF2017_Plot_Locations_Text.png", height=6, width=8, units="in", res=320)
ggplot() +
  geom_raster(data=dem.df, aes(x=x, y=y, fill=ewoods)) +
  geom_polygon(data=ewoods, aes(x=long, y=lat, group=id), fill="green", alpha=0.3) +
  geom_path(data=property, aes(x=long, y=lat, group=group), color="green4", size=2) +
  ...
  # geom_path(data=roads, aes(x=long+5, y=lat+5, group=group), color="ivory", size=0.2) +
  geom_path(data=paths, aes(x=long, y=lat, group=group), color="tan4", size=0.7, linetype="dashed") +
  geom_text(data=selected, aes(x=x, y=y, label=CORNER, color=Type), size=5, fontface="bold") +
  scale_fill_gradient(low="black", high="white", name="Elevation") +
  scale_color_manual(values=c("blue2", "orange3")) +
  coord_equal(xlim=range(dem.df$x), ylim=c(min(dem.df$y), 4631000), expand=F) +
  theme_bw() +
...
write.csv(selected, "data/URF2017_SelectedPlots.csv", row.names=F)
