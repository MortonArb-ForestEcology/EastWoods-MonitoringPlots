...
```{r}
LLNdattot <- bind_rows(LLN2122_clean, LLN18_clean, LLNrun2_clean)
...
```{r}
LLN_trap <- merge(leafTrapTotal, LLN_spec, by = c("year", "plot", "week", ), all.x = T)
dim(LLN_trap)
head(LLN_trap)
masterdf <- merge(LLN_plot, metSummer,  by = c("year"), all.x=T)
dim(masterdf)
head(masterdf)
```

```{r}
lme(N_percent ~ weekPeak * n.Rainless * sci_name, random=list(plot=~1, trap_ID=~1), data=masterdf)
```
